...
typedef struct otr4_conversation_state_t otr4_client_conversation_t;

typedef struct otrv4_client_callbacks_t {
  /* Create a private key for the given accountname/protocol if
   * desired. */
  void (*create_privkey)(
      void *client_opdata); // TODO: This should receive a otr4_client_state_t
  ...
  void (*fingerprint_seen)(const otrv4_fingerprint_t,
                           const otr4_client_conversation_t *);
   ...
   * */
  void (*smp_update)(const otr4_smp_event_t event,
...
void otrv4_client_callbacks_gone_secure(const otrv4_client_callbacks_t *cb,
                                        const otr4_client_conversation_t *conv);

void otrv4_client_callbacks_gone_insecure(
    const otrv4_client_callbacks_t *cb, const otr4_client_conversation_t *conv);
