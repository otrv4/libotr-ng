...
static void
ed448_random_point(decaf_448_point_t p)
{
...
static void
elgamal_448_encrypt(elgamal_448_cipher_t c, decaf_448_scalar_t y, const decaf_448_point_t h, const decaf_448_point_t m)
{
    decaf_448_point_t s;
...
int
dread_encrypt(dread_cipher_t dst, const dread_pubkey_t pub1, const dread_pubkey_t pub2, const unsigned char *msg, size_t msglen, const unsigned char *data, size_t datalen)
{
    gcry_md_hd_t hd;
    ...
    unsigned char *ciphertext = malloc(msglen + crypto_aead_chacha20poly1305_IETF_ABYTES);
    if (!ciphertext)
        return -1;
...
int
dread_decrypt(unsigned char *dst, unsigned long long *dstlen, const dread_keypair_t pair1, const dread_pubkey_t pub2, const dread_cipher_t cipher, const unsigned char *data, size_t datalen)
{
    gcry_md_hd_t hd;
    unsigned char hash[64];
    unsigned char point_buff[DECAF_448_SER_BYTES];
