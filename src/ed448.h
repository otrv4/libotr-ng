...
static inline bool ec_scalar_eq(const ec_scalar_t a, const ec_scalar_t b)
{
	return DECAF_TRUE == decaf_448_scalar_eq(a, b);
}

static inline void ec_point_copy(ec_point_t dst, const ec_point_t src)
{
	decaf_448_point_copy(dst, src);
}

static inline void ec_keypair_generate(ec_keypair_t keypair)
{
	random_bytes(keypair->sym, DECAF_448_SYMMETRIC_KEY_BYTES);
	decaf_448_derive_private_key(keypair, keypair->sym);
}

static inline void ec_keypair_destroy(ec_keypair_t keypair)
{
...
static inline void ec_scalar_copy(ec_scalar_t dst, const ec_scalar_t src)
{
