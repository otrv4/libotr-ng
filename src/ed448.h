...
INTERNAL void ec_bzero(void *data, size_t size);
...
INTERNAL void ec_scalar_destroy(ec_scalar_t dst);

INTERNAL void ec_point_copy(ec_point_t dst, const ec_point_t src);

INTERNAL void ec_point_destroy(ec_point_t dst);

INTERNAL otrv4_bool_t ec_point_valid(const ec_point_t point);

INTERNAL otrv4_bool_t ec_point_eq(const ec_point_t, const ec_point_t);
...
INTERNAL otrv4_bool_t ec_verify(const uint8_t sig[DECAF_EDDSA_448_SIGNATURE_BYTES],
