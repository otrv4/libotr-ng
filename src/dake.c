				    ...
				    const dh_mpi_t their_dh)
{
  ...
  size_t msg_len = our_profile_len
    + their_profile_len + 2 * sizeof (ec_public_key_t) + 2 * DH_MPI_BYTES;
  ...
  uint8_t *cursor = buff;
  cursor +=
    serialize_bytes_array (cursor, their_profile_buff, their_profile_len);
			    ...
			    dake_dre_auth_t * dre_auth)
{
  ...
  if (!sha3_256
      (enc_key, crypto_secretbox_KEYBYTES, k, sizeof (dr_cs_symmetric_key_t)))
    {
      return false;
    }
...
{
  size_t our_profile_len = 0, their_profile_len = 0;
  uint8_t *our_profile_buff = NULL, *their_profile_buff = NULL, *buff = NULL;
  ...
  *dst = buff;
  ...
  cursor += serialize_dh_public_key (cursor, their_dh);
  cursor +=
    serialize_bytes_array (cursor, dre_auth->gamma, sizeof (dre_auth->gamma));
					...
					dake_dre_auth_t * dre_auth)
{
  ...
  if (!dake_dre_auth_generate_phi_message (&phi_msg, &phi_msg_len,
					   ...
					   our_dh, their_profile, their_ecdh,
					   their_dh))
    {
      return false;
    }
			    ...
			    const dake_dre_auth_t * dre_auth)
{
  ...
  bool valid =
    dake_dre_auth_generate_sigma_message (&sigma_msg, NULL, our_profile,
					  our_ecdh_pub, our_dh_pub, dre_auth);
  valid &=
    ring_signature_auth_valid (dre_auth->sigma, sigma_msg,
			       their_profile->pub_key, our_profile->pub_key,
			       y);
			...
			const dake_dre_auth_t * dre_auth)
{
  ...
  if (!dake_dre_auth_verify_sigma
      (dre_auth->profile, our_profile, our_ecdh_pub, our_dh_pub, dre_auth))
    {
      return false;
    }
