...
void
allowed_versions (string_t * dst, const otrv4_t * otr)
				   ...
				   const otrv4_t * otr)
{
  ...
  bool ret =
    serialize_and_encode_identity_message (&response->to_send,
					   identity_message);
...
bool
serialize_and_encode_dre_auth (string_t * dst,
			    ...
			    size_t buflen, otrv4_t * otr)
{
  ...
  if (!retrieve_receiving_message_keys
      (enc_key, mac_key, data_message->ratchet_id, data_message->message_id,
       otr))
    {
...
get_message_type (const string_t message)
{
  if (otrv4_message_contains_tag (message))
    ...
    {				//TODO: not only data, but also DAKE
			 ...
			 size_t message_len, otrv4_t * otr)
{
  ...
  if (!sha3_512_mac
      (ser + bodylen, DATA_MSG_MAC_BYTES, mac_key, sizeof (m_mac_key_t), ser,
       bodylen))
    {
      free (ser);
      return false;
