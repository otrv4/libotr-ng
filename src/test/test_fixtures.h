...
void otrv4_fixture_set_up(otrv4_fixture_t * otrv4_fixture, gconstpointer data)
{
	dh_init();
        ...
        otrv4_fixture->lt_keypair = otrv4_keypair_new();
        otrv4_keypair_generate(otrv4_fixture->lt_keypair);

	otrv4_policy_t policy = {.allows = OTRV4_ALLOW_V4 };
	otrv4_fixture->otr = otrv4_new(otrv4_fixture->lt_keypair, policy);

	otrv4_policy_t policyv3 = {.allows = OTRV4_ALLOW_V3 };
	otrv4_fixture->otrv3 = otrv4_new(otrv4_fixture->lt_keypair, policyv3);

	otrv4_policy_t policyv34 = {.allows = OTRV4_ALLOW_V3 | OTRV4_ALLOW_V4 };
	otrv4_fixture->otrv34 = otrv4_new(otrv4_fixture->lt_keypair, policyv34);
}

void otrv4_fixture_teardown(otrv4_fixture_t * otrv4_fixture, gconstpointer data)
{
    otrv4_keypair_free(otrv4_fixture->lt_keypair);
...
{
    fixture->lt_keypair = otrv4_keypair_new();
        otrv4_keypair_generate(fixture->lt_keypair);
	fixture->profile = user_profile_new("4");
	otrv4_assert(fixture->profile != NULL);
	fixture->profile->expires = time(NULL) + 60 * 60;
	otrv4_assert(user_profile_sign(fixture->profile, fixture->lt_keypair));
}
...
identity_message_fixture_teardown(identity_message_fixture_t * fixture,
				  gconstpointer user_data)
{
        otrv4_keypair_free(fixture->lt_keypair);
