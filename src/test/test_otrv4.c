				     ...
				     gconstpointer data)
{
  ...
  char *query_message = NULL;
  otrv4_build_query_message (&query_message, otrv4_fixture->otrv34, message,
			     strlen (message));
				      ...
				      gconstpointer data)
{
  ...
  char *whitespace_tag = NULL;
  otrv4_build_whitespace_tag (&whitespace_tag, otrv4_fixture->otrv34, message,
			      strlen (message));
  g_assert_cmpstr (whitespace_tag, ==, expected_tag);
  free (whitespace_tag);
}
					      ...
					      gconstpointer data)
{
  ...
  string_t message =
    " \t  \t\t\t\t \t \t \t    \t\t  \t\tAnd some random invitation text.";
  otrv4_assert (otrv4_receive_message
		(response, message, strlen (message), otrv4_fixture->otrv3));
