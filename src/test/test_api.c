...
static void set_up_client_state(otrv4_client_state_t *state,
  ...
  otrv4_client_state_add_private_key_v4(state, sym_key);
  otrv4_client_state_add_shared_prekey_v4(state, sym_key);
  otrv4_client_state_add_instance_tag(state, 0x100 + byte);
}
...
void test_api_conversation_errors(void) {
  ...
  otrv4_assert(err == MSG_NOT_VALID);
  otrv4_assert(response_to_alice->to_send == NULL);
...
void test_api_conversation_v3(void) {
  ...
  otrv4_client_state_add_instance_tag(alice_state, 0x100 + 1);
  otrv4_client_state_add_instance_tag(bob_state, 0x100 + 2);
...
void test_api_smp_abort(void) {
  ...
  otrv4_assert(otrv4_smp_abort(&to_send, bob) == SUCCESS);
  g_assert_cmpint(bob->smp->state, ==, SMPSTATE_EXPECT1);
...
void test_unreadable_flag() {
  ...
  assert_msg_sent(err, to_send);
  otrv4_assert(decoded[flag_position] == MSGFLAGS_IGNORE_UNREADABLE);
  free(decoded);
  decoded = NULL;
