...
void
test_dake_identity_message_new(identity_message_fixture_t *f, gconstpointer data) {
  dake_identity_message_t *identity_message = dake_identity_message_new(f->profile);
  dake_identity_message_free(identity_message);
  identity_message = NULL;
}

void
test_dake_identity_message_serializes(identity_message_fixture_t *f, gconstpointer data) {
  dh_init();
  ...
  ec_public_key_copy(identity_message->Y, ecdh->pub);
  ...
  otrv4_assert(user_profile_aprint(&user_profile_serialized, &user_profile_len, identity_message->profile));
  ...
  ec_public_key_serialize(serialized_y, sizeof(ec_public_key_t), identity_message->Y);
  ...
  size_t mpi_len = dh_mpi_serialize(serialized_b, DH3072_MOD_LEN_BYTES, identity_message->B);
...
void
test_dake_identity_message_deserializes(identity_message_fixture_t *f, gconstpointer data) {
  dh_init();
  ...
  uint8_t *serialized = NULL;
  otrv4_assert(dake_identity_message_aprint(&serialized, &serialized_len, identity_message));

  dake_identity_message_t *deserialized = malloc(sizeof(dake_identity_message_t));
  memset(deserialized, 0, sizeof(dake_identity_message_t));
  otrv4_assert(dake_identity_message_deserialize(deserialized, serialized, serialized_len));

  //assert prekey eq
  g_assert_cmpuint(deserialized->sender_instance_tag, ==, identity_message->sender_instance_tag);
  g_assert_cmpuint(deserialized->receiver_instance_tag, ==, identity_message->receiver_instance_tag);
  otrv4_assert_user_profile_eq(deserialized->profile, identity_message->profile);
...
void
test_dake_identity_message_valid(identity_message_fixture_t *f, gconstpointer data) {
  dh_init();
...
void
test_dake_identity_message_Y_doesnt_belong_to_curve(identity_message_fixture_t *f, gconstpointer data) {
  dake_identity_message_t *identity_message = dake_identity_message_new(f->profile);
  otrv4_assert(identity_message != NULL);
