...
void test_userstate_key_management(void) {
  ...
  otrv4_assert(otrv4_user_state_get_private_key_v4(state, alice_account));
  otrv4_assert(otrv4_user_state_get_private_key_v4(state, bob_account));
  otrv4_assert(!otrv4_user_state_get_private_key_v4(state, charlie_account));
...
static void create_privkey_cb(void *client_id) {
  ...
  otrv4_user_state_add_private_key_v4(test_state, client_id, sym);
}
...
void test_api_messaging(void) {
  ...
  test_state = otrv4_user_state_new(&test_calbacks);

  // This will invoke create_privkey_cb() to create the private keys
  otrv4_assert(otrv4_user_state_get_private_key_v4(test_state, alice_account));
  otrv4_assert(otrv4_user_state_get_private_key_v4(test_state, bob_account));

  otrv4_user_state_free(test_state);
}
