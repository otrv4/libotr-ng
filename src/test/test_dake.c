...
void test_dake_dre_auth_new()
{
	//dake_dre_auth_t *dre_auth = dake_dre_auth_new();
}

void test_dake_generate_gamma_phi_sigma()
{
	bool ok = false;
	...
	ok = dake_dre_auth_generate_gamma_phi_sigma(cs_alice, ecdh_alice->pub,
						    dh_alice->pub, profile_bob,
						    ecdh_bob->pub, dh_bob->pub,
						    dre_auth);
	otrv4_assert(ok);
	...
	ok = dake_dre_auth_validate(received_ecdh_alice, &received_dh_alice,
				    profile_bob, cs_bob, ecdh_bob->pub,
				    dh_bob->pub, dre_auth);
	otrv4_assert(ok);
...
void test_dake_dre_auth_deserialize()
{
	dh_init();
	...
	g_assert_cmpuint(des_dre->sender_instance_tag, ==,
			 dre_auth->sender_instance_tag);
	g_assert_cmpuint(des_dre->receiver_instance_tag, ==,
			 dre_auth->receiver_instance_tag);
